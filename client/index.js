const card = post => {
    ...
    <div class="card z-depth-4">
        ...
        <div class="card-action">
            ...
            <button class="btn-floating btn-small red js-edit" data-id="${post._id}>
  ...
  function renderPosts(_posts = []) {
    ...
    if (_posts.length > 0) {
      $posts.innerHTML = _posts.map(post => card(post)).join(' ')
        ...
        $posts.innerHTML = `
            <div class="center">
                <h1>Тестове завдання для <span style="color:white;">nerdy</span><span style="color: green;">soft</span></h1>
  ...
  function onEditPost(event) {
    if (event.target.classList.contains('js-edit')) {
      const $title = document.querySelector('#title')
      const decision = confirm(`Ви спаравді хочете редагувати ${$title}?`)
    }}